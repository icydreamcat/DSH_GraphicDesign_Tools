...
| **Agent** | `design/` | preset 的规范源：8 工具 + 9 技能 + 常驻纪律 | ✅ |
| **产出** | `examples/`（精选，进仓库）<br>`engine/out/`（成品，不进） | 成品图、效果对照表 | ✅ 精选 |
| **文档** | `docs/`（进仓库：9 份规则 + 2 份方法论）<br>`../knowledge/`（不进，案例与复盘库） | **规则** vs **案例** | 部分 |

> **文档这一条的判据**（2026-09 重定）：**结论性（纯规则）进仓库，可以被人直接照着做；
> "发生过什么"（案例文献、过程记录、本机前提）留本地。**
>
> 于是 `docs/rules/` 是**规则族**——按内容分类，名字是规则的名字；
> 案例文献（别人的作品拆解、课程笔记、实测素材）留在 `../knowledge/reference/`。
> 规则来自案例，但**不被埋在案例里**：一个只想照做的人不该被迫先读 54KB 的实测过程。
...
| 9 个技能与 2 份方法论（`docs/`）、9 份规则（`docs/rules/`） | ✅ 新克隆即可自举 |
