...
**这一节写工具链自身的技术缺口与流程缺口** —— 即代码结构、测试结构、工程流程上的洞。

不写两类东西：

| 不写什么 | 为什么 | 写在哪 |
|---|---|---|
| 本机环境前提、交付记录 | 换机器/换工作区就失效 | `knowledge/tooling/已知缺口与本地前提.md`（不进版本控制） |
| **用户与会话的操作记录** | **那是「用过什么」，不是项目缺口** | 属于运行记录，不属于交付文档 |

### 8.1 CLI 入口本身从未被端到端测试 —— 这是覆盖空缺的根

**技术缺口。** 实测：

```
bin/design.mjs 的子命令：render analyze critique verify fonts ladder ramp palette tool
                          （外加 palette 的 ops/presets/show/apply/run）
16 个测试文件里，真正把 CLI 当程序跑的：0 个
  （run-all.mjs 与 workspace-layout.mjs 会 spawn 进程，但分别是为了「跑套件」和「验证环境变量」）
18 个 src 模块里，被测试直接 import 的：16 个
```

**结构是这样的**：模块级测试很密（416 项断言），**但没有任何测试经由 CLI 走一遍**。
而 agent 的 `design_render` 等 8 个工具，**恰恰是通过 CLI 子进程调用引擎的**。
...
它当前**不可达**（composition 总是显式给 `engineDir`），而且是**兜底回退**、报错信息里
也说明了来源。但它是**作者机器上的路径**，且**本仓库自己的文档刚把这一类字面量定性为缺陷**
（§5.3.3：指向应当是数据，不是硬编码）。别人 clone 之后若那一行被走到，报错会指向一个
他从未听说过的目录。

**修法**：兜底改为「从模块自身位置向上找仓库」（`_preset-locate.mjs` 与各项目脚本已经用了
这个模式），把作者路径彻底移出交付物。

### 8.3 没有任何自动化在跑这套测试

**流程缺口。** 仓库里**没有 CI 配置**（无 `.github/workflows`、无其他 CI 文件），
**也没有根 `package.json`**：测试入口只在 `engine/` 内（`cd engine; node test/run-all.mjs`）。

后果是具体的：`run-all.mjs` 的设计意图是「一条命令、一个退出码」，
但**没有任何东西强制它在每次改动后被执行**。本仓库已经发生过一次「守卫过期，8/8 通过却 exit 1
而长期无人发现」——那条教训促成了 `run-all.mjs`，而**它的执行仍然是自愿的**。

**修法**：加一个最小 CI（GitHub Actions 一个 job：`npm install` → `node test/run-all.mjs`），
或至少在根加 `package.json` 把入口统一，让「跑测试」不需要先记住 `cd engine`。
